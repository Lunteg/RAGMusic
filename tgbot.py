...
class MusicBot:
    ...
    def find_similar_tracks(self, data, user_indices, n_neighbors=5):
        ...
        distances = pairwise_distances(
            avg_vector, X_projected, metric='euclidean')

        # Сортируем и выбираем n_neighbors ближайших треков
        # +1 чтобы исключить сам трек
        similar_indices = np.argsort(distances[0])[:n_neighbors + 1]
        # Исключаем сам трек из результатов
    ...
    def generate_recommendations(self, user_tracks_id):
        ...
        tracks_names = [
            f"{row['track_name']} by {row['artist_name']}" for _, row in user_tracks_list.iterrows()]
